...
func testRocksDbDatastore() (DataStore, error) {
...
func TestRocksDbStore_Set(t *testing.T) {
	ds, err := testRocksDbDatastore()
	assert.NoError(t, err)
	type kv struct {
		key string
		val string
...
func TestRocksDbStore_Get(t *testing.T) {
	type kv struct {
		key string
		val string
	}
...
func TestRocksDbStore_SetUint64(t *testing.T) {
	ds, err := testRocksDbDatastore()
	assert.NoError(t, err)
	type kv struct {
		key string
...
func TestRocksDbStore_GetUint64(t *testing.T) {
	type kv struct {
		key string
		val uint64
	}
